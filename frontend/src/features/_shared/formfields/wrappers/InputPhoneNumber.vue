	...
	<div class="field w-full mt-6">
		<div class="w-full">
			<input
				...
				@input="($e) => (modelValue = iti.getNumber())"
				@countrychange="($e: any) => (modelValue = iti.getNumber())"
...
rules.phoneNumber = withI18nMessage(
	helpers.withParams({ label: props.label, rules }, (value) => {
...
onMounted(() => {
	...
	if (modelValue.value) iti.setNumber(modelValue.value as string)
...
const updateNumber = (number: string | null) => {
	if (mounted && number !== null) iti.setNumber(number)
