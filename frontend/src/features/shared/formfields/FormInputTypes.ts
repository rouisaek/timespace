...
export function getRules(
  ...
  if (props.between !== undefined)
    rules.between = withI18nMessage(
      helpers.withParams(
        { label },
        betweenRule(props.between.min, props.between.max)
      ),
      { withArguments: true }
    );

  if (props.decimal)
    rules.decimal = withI18nMessage(
      helpers.withParams({ label }, decimalRule),
      { withArguments: true }
    );
  ...
  if (props.maxDate !== undefined)
    rules.maxDate = withI18nMessage(
      helpers.withParams(
        { label },
        (value: Dayjs) => value.toDate() <= props.maxDate!.toDate()
      ),
      { withArguments: true }
    );
  ...
  if (props.maxValue !== undefined)
    rules.maxValue = withI18nMessage(
      helpers.withParams({ label }, maxValueRule(props.maxValue)),
      { withArguments: true }
    );
  ...
  if (props.minDate !== undefined)
    rules.maxDate = withI18nMessage(
      helpers.withParams(
        { label },
        (value: Dayjs) => value.toDate() >= props.minDate!.toDate()
      ),
      { withArguments: true }
    );
  ...
  if (props.minValue !== undefined)
    rules.minValue = withI18nMessage(
      helpers.withParams({ label }, minValueRule(props.minValue)),
      { withArguments: true }
    );

  if (props.numeric)
    rules.numeric = withI18nMessage(
      helpers.withParams({ label }, numericRule),
      { withArguments: true }
    );

  if (props.required)
    rules.required = withI18nMessage(
      helpers.withParams({ label }, requiredRule),
      { withArguments: true }
    );
