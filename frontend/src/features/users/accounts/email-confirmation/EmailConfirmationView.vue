...
onMounted(() => {
	...
	apiClient.post("/accounts/email-confirmation", {
		email,
		code: token
	}).then(() => {
		success.value = true;
	...
	<div class="flex w-full h-full place-items-center justify-center gradient-bg">
		<div
			...
			<template v-if="success">
				<Message severity="success" class="mb-4">{{ $t("confirmEmailPage.successMessage") }}
				</Message>
				<RouterLink class="text-indigo-700 dark:text-indigo-300 font-semibold" :to="{ name: 'login' }">
					{{ $t("confirmEmailPage.loginText") }} </RouterLink>
			</template>
			<Message v-if="error" severity="error" class="mb-4">{{ $t("confirmEmailPage.errorMessage") }}</Message>
		</div>
	</div>
</template>
