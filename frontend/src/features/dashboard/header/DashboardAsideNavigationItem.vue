...
const menuItemStyle = "text-tprimary cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700 flex flex-row gap-2 items-center p-2 rounded"
...
const allowedMenuItem = computed(() => {
	...
	console.log(props.item?.label, currentItemAndChildPermissions);
...
<template>
	<li :class="{ 'layout-root-menuitem': root, 'active-menuitem': isActiveMenu }" v-if="item && allowedMenuItem">
		<div v-if="root && item.visible !== false && item.label.length > 0"
			class="text-tsecondary font-bold text-sm mt-4">{{
				item.label.toUpperCase()
			}}</div>
		<a v-if="(!item.to || item.items) && item.visible !== false && !root" @click="itemClick($event, item)"
			:class="[item.class, menuItemStyle]" tabindex="0">
		...
		<router-link v-if="item.to && !item.items && item.visible !== false" @click="itemClick($event, item)"
			:class="[item.class, { 'bg-indigo-50 dark:bg-slate-800': checkActiveRoute(item) }, menuItemStyle]"
			tabindex="0" :to="{ name: item.to }">
			<iconify-icon v-if="item.icon" :icon="item.icon" height="none" class="h-6 w-6" />
			<span class="flex-grow">{{ item.label }}</span>
			<iconify-icon icon="heroicons:chevron-right" v-if="item.items && !isActiveMenu" />
			<iconify-icon icon="heroicons:chevron-down" v-if="item.items && isActiveMenu" />
		...
		<Transition v-if="item.items && item.visible !== false" name="layout-submenu">
			<ul v-show="root ? true : isActiveMenu" :class="{ 'ml-2': item.label.length > 0 }">
				<DashboardAsideNavigationItem v-for="(child, i) in item.items" :key="child.label" :index="i"
					:item="child" :parentItemKey="itemKey" :root="false" />
			</ul>
		</Transition>
	</li>
</template>
